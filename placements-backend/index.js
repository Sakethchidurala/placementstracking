...
app.post('/addrecruiter', async (req, res) => {//adds data  to the database placements in th collection recruiters
    try {
        const database = db.getDb().db("Placements")
...
//dashboard stats,upcomming rounds,company wise
app.get("/", async (req, res) => {
    ...
    try {
        ...
        for (i = 0; i < result1.length; i++) {
            ...
            let k = await database.collection('recruiters').findOne({ _id: ObjectId(result1[i].recruiter_id )})
            if(k!=null){
                console.log(k)
